...
fn test_op_numnotequal_true() {
    let program = "OP_1 OP_2 OP_NUMNOTEQUAL";
    let mut compiler = CompilerTraitImpl::new();
    let bytecode = compiler.compile(program);
    let mut engine = EngineTraitImpl::new(bytecode);
    ...
    let _ = engine.step();
    let res = engine.step();
    assert!(res, "Execution of OP_NUMNOTEQUAL failed for 1 != 2");

    let dstack = engine.get_dstack();
    assert_eq!(dstack.len(), 1, "Stack length is not 1 for 1 != 2");
...
fn test_op_numnotequal_false() {
    let program = "OP_1 OP_1 OP_NUMNOTEQUAL";
    let mut compiler = CompilerTraitImpl::new();
    let bytecode = compiler.compile(program);
    let mut engine = EngineTraitImpl::new(bytecode);
    ...
    let _ = engine.step();
    let res = engine.step();
    assert!(res, "Execution of OP_NUMNOTEQUAL failed for 1 != 1");

    let dstack = engine.get_dstack();
    assert_eq!(dstack.len(), 1, "Stack length is not 1 for 1 != 1");

    let expected_stack = array![""];
    assert_eq!(dstack, expected_stack.span(), "Stack is not equal to expected for 1 != 1");
}
