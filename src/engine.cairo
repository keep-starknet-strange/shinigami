...
pub trait EngineTrait {
    ...
    fn pull_data_at(ref self: Engine, idx: usize, len: usize) -> Result<ByteArray, felt252>;
    // Pop bool enforcing minimal if
    fn pop_if_bool(ref self: Engine) -> Result<bool, felt252>;
    ...
    fn push_data_len(ref self: Engine, opcode: u8, idx: u32) -> Result<usize, felt252>;
    ...
    fn verify_witness(ref self: Engine, witness: Span<ByteArray>) -> Result<(), felt252>;
...
pub impl EngineImpl of EngineTrait {
    ...
    fn pop_if_bool(ref self: Engine) -> Result<bool, felt252> {
        ...
        return Result::Ok(utils::byte_array_to_bool(@top));
    }
    ...
    fn push_data_len(ref self: Engine, opcode: u8, idx: u32) -> Result<usize, felt252> {
        if opcode == Opcode::OP_PUSHDATA1 {
            return Result::Ok(
                utils::byte_array_to_felt252_le(@self.pull_data_at(idx + 1, 1)?).try_into().unwrap()
            );
        } else if opcode == Opcode::OP_PUSHDATA2 {
            return Result::Ok(
                utils::byte_array_to_felt252_le(@self.pull_data_at(idx + 1, 2)?).try_into().unwrap()
            );
        } else if opcode == Opcode::OP_PUSHDATA4 {
            return Result::Ok(
                utils::byte_array_to_felt252_le(@self.pull_data_at(idx + 1, 4)?).try_into().unwrap()
            );
        }
    ...
    fn verify_witness(ref self: Engine, witness: Span<ByteArray>) -> Result<(), felt252> {
        if self.is_witness_active(0) {
            ...
            if self.witness_program.len() == 20 {
                ...
                let mut pk_script = utils::hex_to_bytecode(@"0x76a914");
                pk_script.append(@self.witness_program);
                pk_script.append(@utils::hex_to_bytecode(@"0x88ac"));
                ...
                let witness_hash = utils::sha256_byte_array(witness_script);
