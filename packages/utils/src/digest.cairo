...
/// `Into` implementation that converts a `Digest` value into a `@ByteArray`.
pub impl DigestIntoByteArrayImpl of Into<@Digest, @ByteArray> {
    fn into(self: @Digest) -> @ByteArray {
        let mut bytes: ByteArray = Default::default();
        for word in self.value.span() {
            bytes.append_word((*word).into(), 4);
...
//             value: [
//                 0xefcdab90, 0x78563412, 0xefcdab90, 0x78563412, 0x21436587, 0x09badcfe,
//                 0x21436587, 0x09badcfe,
//             ],
//         };
...
//             value: [
//                 0xfedcba09, 0x87654321, 0xfedcba09, 0x87654321, 0x12345678, 0x90abcdef,
