use shinigami_engine::transaction::EngineInternalTransactionTrait;
use crate::utxo::UTXO;
use crate::validate;
use shinigami_utils::bytecode::hex_to_bytecode;

#[test]
fn test_p2wpkh_transaction() {
    // https://learnmeabitcoin.com/explorer/tx/c178d8dacdfb989f9d4fa45828ed188cd54a0414d625c3e61e75c5e3ac15a83a#output-0
    let raw_transaction_hex =
        "";
    let raw_transaction = hex_to_bytecode(@raw_transaction_hex);
    let transaction = EngineInternalTransactionTrait::deserialize(raw_transaction);

    let prevout_pk_script = "0x0014841b80d2cc75f5345c482af96294d04fdd66b2b7";
    let prevout = UTXO {
        amount: 1083200, pubkey_script: hex_to_bytecode(@prevout_pk_script), block_height: 123456
    };

    let utxo_hints = array![prevout];
    let res = validate::validate_transaction(@transaction, 0, utxo_hints);

    assert!(res.is_ok(), "Transaction validation failed");
}

#[test]
fn test_p2wpkh_witness_unlocking() {
    // https://learnmeabitcoin.com/explorer/tx/1674761a2b5cb6c7ea39ef58483433e8735e732f5d5815c9ef90523a91ed34a6#input-0
    let raw_transaction_hex =
        "";
    let raw_transaction = hex_to_bytecode(@raw_transaction_hex);
    let transaction = EngineInternalTransactionTrait::deserialize(raw_transaction);

    let prevout_pk_script = "0x001430cd68883f558464ec7939d9f960956422018f07";
    let prevout = UTXO {
        amount: 1068340, pubkey_script: hex_to_bytecode(@prevout_pk_script), block_height: 123456
    };
    let utxo_hints = array![prevout];

    let res = validate::validate_transaction(@transaction, 0, utxo_hints);
    assert!(res.is_ok(), "Transaction validation failed");
}


#[test]
fn test_p2wpkh_first_transaction() {
    // https://learnmeabitcoin.com/explorer/tx/dfcec48bb8491856c353306ab5febeb7e99e4d783eedf3de98f3ee0812b92bad#input-0
    let raw_transaction_hex =
        "";
    let raw_transaction = hex_to_bytecode(@raw_transaction_hex);
    let transaction = EngineInternalTransactionTrait::deserialize(raw_transaction);

    let prevout_script = "0x00148d7a0a3461e3891723e5fdf8129caa0075060cff";
    let prevout = UTXO {
        amount: 194300, // Specify amount in satoshis
        pubkey_script: hex_to_bytecode(@prevout_script),
        block_height: 481824
    };
    let utxo_hints = array![prevout];

    let res = validate::validate_transaction(@transaction, 0, utxo_hints);
    assert!(res.is_ok(), "P2WPKH Transaction validation failed");
}

#[test]
fn test_p2wpkh_first_witness_spend() {
    // https://learnmeabitcoin.com/explorer/tx/f91d0a8a78462bc59398f2c5d7a84fcff491c26ba54c4833478b202796c8aafd#input-0
    let raw_transaction_hex =
        "";
    let raw_transaction = hex_to_bytecode(@raw_transaction_hex);
    let transaction = EngineInternalTransactionTrait::deserialize(raw_transaction);

    let prevout_script = "0x00148d7a0a3461e3891723e5fdf8129caa0075060cff";
    let prevout = UTXO {
        amount: 81530, pubkey_script: hex_to_bytecode(@prevout_script), block_height: 481824
    };
    let utxo_hints = array![prevout];

    let res = validate::validate_transaction(@transaction, 0, utxo_hints);
    assert!(res.is_ok(), "P2WPKH first follow-up spend witness validation failed");
}

#[test]
fn test_p2wpkh_uncompressed_key_scriptpubkey_validation() {
    // https://learnmeabitcoin.com/explorer/tx/7c53ba0f1fc65f021749cac6a9c163e499fcb2e539b08c040802be55c33d32fe#input-0
    let raw_transaction_hex =
        "";
    let raw_transaction = hex_to_bytecode(@raw_transaction_hex);
    let transaction = EngineInternalTransactionTrait::deserialize(raw_transaction);

    let prevout_script = "0x0014671041727b982843f7e3db4669c2f542e05096fb";
    let prevout = UTXO {
        amount: 90000, pubkey_script: hex_to_bytecode(@prevout_script), block_height: 481824
    };
    let utxo_hints = array![prevout];

    let res = validate::validate_transaction(@transaction, 0, utxo_hints);
    assert!(res.is_ok(), "P2WPKH uncompressed key ScriptPubKey validation failed");
}
