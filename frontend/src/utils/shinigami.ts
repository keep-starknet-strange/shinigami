...
class ScriptNum {
  ...
  static fromBuffer(
    buf: Buffer,
    fRequireMinimal: boolean = true,
    nMaxNumSize: number = ScriptNum.MAX_NUM_SIZE,
  ): bigint {
...
class SignatureChecker {
  ...
  static verifyMultisig(
    ...
    while (sigIndex < signatures.length && keyIndex < pubKeys.length) {
      if (
        this.verifySignature(pubKeys[keyIndex], signatures[sigIndex], message)
      ) {
        sigIndex++;
      }
      keyIndex++;
    ...
    switch (opcode) {
      ...
      case Opcode.OP_NOTIF: {
        ...
        this.conditionalStack.push(
          opcode === Opcode.OP_IF ? condition : !condition,
        );
        break;
      }
      ...
      case Opcode.OP_CHECKSIG: {
        ...
        const success = SignatureChecker.verifySignature(
          pubkey,
          signature,
      ...
      case Opcode.OP_CHECKMULTISIG: {
        ...
        // Verify signatures
        const success = SignatureChecker.verifyMultisig(
          pubkeys,
          signatures,
          this.getSignatureHash(),
        );
      ...
      case Opcode.OP_CHECKMULTISIGVERIFY: {
        ...
        if (
          !SignatureChecker.verifyMultisig(
            pubkeys,
            signatures,
            this.getSignatureHash(),
          )
...
function backendRun(input: InputData): number {
  console.log(
    `Running Bitcoin Script with ScriptSig: '${input.ScriptSig}' and ScriptPubKey: '${input.ScriptPubKey}'`,
  );
